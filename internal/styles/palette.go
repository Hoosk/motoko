...
// palette holds the 13 theme colors as a single value so themes can be
// declared as data instead of a long switch.
type palette struct {
	Background         lipgloss.Color
	Surface            lipgloss.Color
	SurfaceSoft        lipgloss.Color
	MainNeon           lipgloss.Color
	AccentBlue         lipgloss.Color
	AccentViolet       lipgloss.Color
	Gray               lipgloss.Color
...
// palettes is the set of selectable themes. The cyberpunk theme is the
// default and is applied when an unknown name is requested.
...
// applyPalette writes a palette into the mutable theme color variables.
...
// reinitStyles rebuilds every style instance from the current color
// variables. It must be called after the palette changes.
func reinitStyles() {
	reinitCoreStyles()
	reinitPopupStyles()
	reinitContextStyles()
